...
export function M3Pane(props: { children: React.JSX.Element, lastElement?: boolean, style?: React.CSSProperties }) {
    ...
    return (
        <Fade in>
            <div style={{borderRadius: 28, ...(isMediumOrAbove && {backgroundColor: `var(--md-sys-color-surface)`}), marginTop: 16, marginBottom: 16, padding: 24, ...(lastElement && isMediumOrAbove && {marginRight: '1rem'}), ...(style)}}>
                {children}
            </div>
        </Fade>
    );
